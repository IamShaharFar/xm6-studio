...
/// Sony request/reply layouts adapted from the pinned MIT xm6-control source.
/// Additional modern noise-control layout reviewed against SonyHeadphonesClient.
/// See THIRD_PARTY.md for revisions and attribution. Hardware confirmation is
/// tracked separately in Verification.md; transport ACKs alone never confirm writes.
public enum SonyCommands {
    ...
    public static func protocolVersion(fromInitReplyPayload payload: [UInt8]) -> ProtocolVersion {
        ...
        case 4 where payload[1] == 0: return .v1
        case 8 where payload[1] == 0 && payload[6] <= 1 && payload[7] <= 1: return .v2
    ...
    public static func buildAmbientSoundSet(_ state: AmbientSoundState) -> [UInt8] {
        ...
        if state.subtype == 0x19, let enabled = state.adaptiveEnabled, let sensitivity = state.adaptiveSensitivity {
    ...
    /// Supports legacy 0x15/0x17/0x22 and modern 0x19 with adaptive-ambient fields.
    /// The latter is confirmed on XM6 firmware 3.1.5 and takes precedence in state.
    public static func decodeAmbientSound(_ payload: [UInt8]) -> AmbientSoundState? {
        guard payload.count >= 7, payload.count <= 9 else { return nil }
        let subtype = payload[1]
        guard subtype == 0x15 || subtype == 0x17 || subtype == 0x19 || subtype == 0x22 else { return nil }
        if subtype == 0x19 {
            guard payload.count == 9, payload[7] <= 1, payload[8] <= 2 else { return nil }
        } else if payload.count > 8 { return nil }
        ...
        return AmbientSoundState(
            ...
            adaptiveEnabled: subtype == 0x19 ? payload[7] == 1 : nil,
            adaptiveSensitivity: subtype == 0x19 ? payload[8] : nil
        )
    }
    ...
    public static func decodeBattery(_ payload: [UInt8]) -> BatteryStatus? {
        guard payload.count >= 4, payload[1] == 0x00, payload[2] <= 100, payload[3] <= 1 else { return nil }
    ...
    public static func decodeEqualizer(_ payload: [UInt8]) -> EqualizerState? {
        ...
        guard bandCount <= 32, payload.count >= 4 + bandCount else { return nil }
        var bands: [Int] = []
    ...
    public static func decodeBGMMode(_ payload: [UInt8]) -> (Bool, BGMRoomSize)? {
        guard payload.count >= 4, payload[1] == 0x09 || payload[1] == 0x03, payload[2] <= 1 else { return nil }
    ...
    public static func decodeUpmixCinema(_ payload: [UInt8]) -> Bool? {
        guard payload.count >= 3, payload[1] == 0x04, payload[2] <= 1 else { return nil }
    ...
    public static func buildSourceSwitchSet(macAddress: String) -> [UInt8]? {
        ...
        guard macBytes.count == 17, isBluetoothAddress(macAddress) else { return nil }
    ...
    public static func decodeDeviceList(_ payload: [UInt8]) -> [MultipointDevice]? {
        ...
        if count == 0 { return payload.count == 4 ? [] : nil }
        ...
        for _ in 0..<count {
            ...
            guard let mac = String(bytes: payload[i..<(i + 17)], encoding: .utf8), isBluetoothAddress(mac) else { return nil }
            i += 17
            guard i < payload.count - 1 else { return nil }
            ...
            i += 1
            guard i + nameLen <= payload.count - 1 else { return nil }
            guard let name = String(bytes: payload[i..<(i + nameLen)], encoding: .utf8) else { return nil }
            i += nameLen
