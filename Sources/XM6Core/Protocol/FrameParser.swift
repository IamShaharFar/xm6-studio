...
public final class FrameParser {
    ...
    public func feed(_ data: [UInt8]) -> [SonyMessage] {
        ...
        while true {
            ...
            // A new unescaped header means the prior frame was truncated.
            if let nextHeader = buffer.dropFirst().firstIndex(of: SonyMessage.header),
               buffer.firstIndex(of: SonyMessage.trailer).map({ nextHeader < $0 }) ?? true {
                buffer.removeFirst(nextHeader)
                continue
            }
