...
public struct SonyMessage: Equatable, Sendable {
    ...
    public static func decode(rawFrame: [UInt8]) -> SonyMessage? {
        ...
        var index = 1
        while index < rawFrame.count - 1 {
            let byte = rawFrame[index]
            if byte == escapeMarker {
                index += 1
                guard index < rawFrame.count - 1, [UInt8(0x2c), 0x2d, 0x2e].contains(rawFrame[index]) else { return nil }
            } else if byte == header || byte == trailer { return nil }
            index += 1
        }
        ...
        guard sequenceNumber <= 1 else { return nil }
        ...
        guard type != .ack || payload.isEmpty else { return nil }
